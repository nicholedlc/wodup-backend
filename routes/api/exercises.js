...
router.get("/", function(req, res, next) {
  Exercise.findAll({ order: [["name", "ASC"]] })
    .then(exercises => {
      res.json({ exercises });
    })
    .catch(err => {
...
router.post("/", function(req, res, next) {
  ...
  Exercise.create({ name, description })
...
router.get("/:id", function(req, res, next) {
  ...
  Exercise.findById(id, {
    ...
    order: [[{ model: Log, as: "logs" }, "date", "DESC"]]
  })
    .then(exercise => res.json({ exercise }))
    .catch(err => {
      res.json({ err: { name: err.name, message: err.message } });
...
router.delete("/:id", function(req, res, next) {
  ...
  Exercise.findById(id)
    .then(exercise => {
      exercise.destroy().then(() => res.json({ exercise }));
    })
    .catch(err => {
      res.json({ err: { name: err.name, message: err.message } });
    });
...
router.post("/:id/log", function(req, res, next) {
  ...
  Exercise.findById(id)
    .then(exercise =>
      exercise.createLog({ rep, set, weight, note, date, imageUrl })
    )
